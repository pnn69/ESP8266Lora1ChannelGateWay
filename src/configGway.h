...
//
// Based on work done by Thomas Telkamp for Raspberry PI 1ch gateway and many
// others.
//
...
//
// This file contains a number of compile-time settings that can be set on (=1)
// or off (=0) The disadvantage of compile time is minor compared to the memory
// gain of not having too much code compiled and loaded on your ESP device.
//
// NOTE:
...
//
// The source has been optimized for PlatformIO usage. Than means the some
// variables can be define in the .platformio.ini file. Pleas look at the
// example file to see how to set these #defines
//
// ========================================================================================
...
// Define the monitor screen. When it is greater than 0 then logging is
// displayed in the special screen at the GUI. If _DUSB is also set to 1 then
// most messages will also be copied to USB devices.
#define _MONITOR 1
...
// 3= See Option 2, but with extra channel info (Not used when Hopping is not
// selected)
...
// Define whether to use the old Semtech gateway API, which is still supported
// by TTN, but is more lightweight than the new TTN tcp based protocol. NOTE:
// Only one of the two should be defined! TTN Router project has stopped
//
#define _UDPROUTER 1
//#define _TTNROUTER 1

// The spreading factor is the most important parameter to set for a single
// channel gateway. It specifies the speed/datarate in which the gateway and
// node communicate. As the name says, in principle the single channel gateway
// listens to one channel/frequency and to one spreading factor only. This
// parameters contains the default value of SF, the actual version can be set
// with the webserver and it will be stored in SPIFF NOTE: The frequency is set
// in the loraModem.h file and is default 868.100000 MHz.
...
// 1-channel gateway will become even more versatile. If set to 0 we will use
// the continuous listen mode. Using this function means that we HAVE to use
// more dio pins on the RFM95/sx1276 device and also connect enable dio1 to
// detect this state.
#define _CAD 1

// CRCCHECK
// Defines whether we should check on the CRC of RXDONE messages (see
// stateMachine.ino) This should prevent us from getting a lot os stranges
// messgages of unknown nodes. Note: DIO3 must be connected for this to work
// (Heltec and later Wemos gateways).
#define _CRCCHECK 1

// Definitions for the admin webserver.
// A_SERVER determines whether or not the admin webpage is included in the
// sketch. Normally, leave it in!
...
// Make sure that tou have installed Python version 2.7 and have Bonjour in your
// network. Bonjour is included in iTunes (which is free) and OTA is recommended
// to install the firmware on your router witout having to be really close to
// the gateway and connect with USB.
#define A_OTA 1

// We support a few pin-out configurations out-of-the-box: HALLARD, COMPRESULT
// and TTGO ESP32. If you use one of these two, just set the parameter to the
// right value. If your pin definitions are different, update the loraModem.h
// file to reflect these settings.
//	1: HALLARD
//	2: COMRESULT pin out
//	3: ESP32, Wemos pin out (Not used)
//	4: ESP32, Heltec and TTGO pin out (should work for Heltec, 433 and OLED
// too). 	5: Other, define your own in loraModem.h (does not include GPS
// Code)
#if !defined _PIN_OUT
#define _PIN_OUT 1
#endif
...
// If the 1ch gateway is working in and for nodes that ONLY transmit and receive
// on the set and agreed frequency and spreading factor. make sure to set STRICT
// to 1. In this case, the frequency and spreading factor for downlink messages
// is adapted by this gateway NOTE: If your node has only one frequency enabled
// and one SF, you must set this to 1
...
// Define if OLED Display is connected to I2C bus. Note that defining an OLED
// display does not impact performance negatively, certainly if no OLED is
// connected. Wrong OLED will not show sensible results on the OLED display
...
// thru webinterface).
// This will allow us to send messages over the UDP connection to manage the
// gateway and its parameters. Sometimes the gateway is not accesible from
// remote, in this case we would allow it to use the SERVER UDP connection to
// receive messages as well. NOTE: Be aware that these messages are NOT LoRa and
// NOT LoRa Gateway spec compliant.
//	However that should not interfere with regular gateway operation but
// instead offer 	functions to set/reset certain parameters from remote.
#define _GATEWAYMGT 0

// Do extensive logging
// Use the ESP8266 SPIFS filesystem to do extensive logging.
// We must take care that the filesystem never(!) is full, and for that purpose
// we rather have new records/line of statistics than very old. Of course we
// must store enough records to make the filesystem work
#define _STAT_LOG 1

// Set the Server Settings (IMPORTANT)
#define _LOCUDPPORT \
  1700  // UDP port of gateway! Often 1700 or 1701 is used for upstream comms

// Timing
#define _MSG_INTERVAL 15  // Reset timer in seconds
#define _PULL_INTERVAL \
  55  // PULL_DATA messages to server to get downstream in milliseconds
...
#define _DESCRIPTION "NicE 1 channel gateway"  // Name of the gateway
#define _EMAIL "pnn_69@hotmail.com"            // Owner
#define _PLATFORM "ESP8266"
#define _LAT 52.32044726
#define _LON 4.96572066
#define _ALT 15
...
#define NTP_TIMEZONES \
  1  // How far is our Timezone from UTC (excl daylight saving/summer time)
...
// after all, a gateway can be a node to the system as well. Some sensors like
// GPS can be sent to the backend as a parameter, some (like humidity for
// example) can only be sent as a regular sensor value. Set its LoRa address and
// key below in this file, See spec. para 4.3.2 NOTE: The node is switched off
// by default. Switch it on in the GUI
...
// Maximum number of Message History statistics records gathered. 20 is a good
// maximum (memory intensive). For ESP32 maybe 30 could be used as well
#define _MAXSTAT 20
