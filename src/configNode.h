...
//
// This file contains a number of compile-time settings and declarations that
// are' specific to the LoRa rfm95, sx1276, sx1272 radio of the gateway.
//
//
// ------------------------------------------------------------------------------------
...
// In some cases we like to decode the lora message at the single channel
// gateway. In thisase, we need the NkwSKey and the AppsSKey of the node so that
// we can decode its messages. Although this is probably overkill in normal
// gateway situations, it greatly helps in debugging the node messages before
// they reach the TTN severs.
//
#if _LOCALSERVER == 1
struct codex {
...
// #define _THINGSERVER "your.extra.server"		// Server URL of the
// LoRa-udp.js handler
// #define _THINGPORT 1700						// Port 1700
// is old compatibility

// Wifi definitions
// WPA is an array with SSID and password records. Set WPA size to number of
// entries in array When using the WiFiManager, we will overwrite the first
// entry with the accesspoint we last connected to with WifiManager NOTE:
// Structure needs at least one (empty) entry.
...
wpas wpa[] = {{"NicE_Engineering_UPC", "1001100110"},
              {"airsupplies", "02351228133648477429"}};

// Please fill in at least ONE valid SSID and password from your own WiFI
// network below. This is needed to get the gateway working
//
// wpas wpa[] = {
...
#define ASSERT(cond) \
  if (!(cond)) gway_failed(__FILE__, __LINE__)
#else
#define ASSERT(cond) /**/
#endif
