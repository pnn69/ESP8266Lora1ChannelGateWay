...
//
// Based on work done by Thomas Telkamp for Raspberry PI 1-ch gateway and many
// others.
//
...
//
// 2. Semtech Basic communication protocol between Lora gateway and server
// version 3.0.0
...
time_t startTime =
    0;  // The time in seconds since 1970 that the server started. We use this
        // variable since millis() is reset every 50 days...
...
#define STATUS_SIZE \
  512  // Should(!) be enough based on the static text .. was 1024
...
void printIP(IPAddress ipa, const char sep,
             String &response);  // _wwwServer.ino
...
int sendUdp(IPAddress server, int port, uint8_t *msg,
            int length);  // _udpSemtech.ino
...
void setup() {
  ...
  // Read the config file for all parameters not set in the setup() or
  // configGway.h file This file should be read just after SPIFFS is initializen
  // and before other configuration parameters are used.
  //
  ...
  sprintf(MAC_char, "%02x:%02x:%02x:%02x:%02x:%02x", MAC_array[0], MAC_array[1],
  ...
  // Setup WiFi UDP connection. Give it some time and retry x times. '0' means
  // try forever
  ...
  // After there is a WiFi router connection, we set the hostname with last 3
  // bytes of MAC address.
#if defined(ESP32_ARCH)
  // ESP32
  sprintf(hostname, "%s%02x%02x%02x", "esp32-", MAC_array[3], MAC_array[4],
          MAC_array[5]);
  WiFi.setHostname(hostname);
  MDNS.begin(hostname);
#else
  // ESP8266
  sprintf(hostname, "%s%02x%02x%02x", "esp8266-", MAC_array[3], MAC_array[4],
          MAC_array[5]);
...
void loop() {
  ...
  // and monitoring of the node. This function is important so it is called at
  ...
  // messages on UDP for every message sent by the gateway. So we have to
  // consume them. As we do not know when the server will respond, we test in
  // every loop.
  //
  else {
    while ((packetSize = Udp.parsePacket()) > 0) {
...
#endif  //_MONITOR
      // DOWNSTREAM
      // Packet may be PKT_PUSH_ACK (0x01), PKT_PULL_ACK (0x03) or PKT_PULL_RESP
      // (0x04) This command is found in byte 4 (buffer[3])
      if (readUdp(packetSize) < 0) {
#if _MONITOR >= 1
        if (debug >= 0) mPrint("readUdp ERROR,down returning < 0");
#endif  //_MONITOR
        break;
      }
      ...
      // otherwise a UDP message with other TTN content
        ...
        //_event=1;
        //// Could be done double if more messages received
      }
    }
  }
...
#if _MONITOR >= 1
        if ((debug >= 1) && (pdebug & P_MAIN)) {
          ntptimer = nowSeconds;  // Do not when year(now())=="1970" beacause of
                                  // "FORMAT"
        }
#endif
      }
